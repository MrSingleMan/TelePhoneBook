...
def copy_row_to_file(filename):
    filename2 = "phone2.csv"
    if not exists(filename2):
        create_file(filename2)
    row_number = int(input("Введите номер строки: "))
    res = read_file(filename)
    if row_number - 1 >= len(res):
        print("Такой строки нету")
        return
    res_copy = res[row_number - 1]
    res2 = read_file(filename2)
    res2.append(res_copy)
    standard_write(filename2, res2)
    standard_write(filename, res)
